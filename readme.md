...
CLI tooling for opening and handling a CLI session on a Prisma SD-WAN ION without using the Strata Cloud GUI. It hits the same SASE API the browser does: a REST call to look up the element, then a websocket into the cli session.

The SCM UI is the only supported way to reach an ION's cli without local access. That works for one device. It doesn't script and it doesn't scale. This does.

## Requirements

- Network access to `api.sase.paloaltonetworks.com` over 443 (HTTPS and WSS)
- A Prisma SD-WAN bearer token
- CLI credentials for the element if you're using `-cmd`
